...
function openIDEByNextLaunchEditor(debugSource: DebugSource) {
  ...
  const debugUrl = `http://localhost:3000/__nextjs_launch-editor?file=${debugSource.fileName}&lineNumber=${debugSource.lineNumber}&column=${debugSource.columnNumber}`;
  fetch(debugUrl);
